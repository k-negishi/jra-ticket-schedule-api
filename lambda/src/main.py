...
from src.utils.response import create_error_response, create_response
    ...
    try:
        ...
        if path == "/jra-calendar/events" and http_method == "GET":
            ...
            return create_response(
                200, {"events": formatted_races, "date": f"{year:04d}-{month:02d}-{day:02d}", "count": len(races)}
            )
...
def _format_and_sort_races(races):
    ...
    return [{"race_name": event.get("summary", ""), "location": event.get("location", "")} for event in sorted_events]
